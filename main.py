...
parser.add_argument('-epstol',     help='Error tolerance')
...
parser.add_argument('-adaptauto',  help='adaptivity: enable adapt mesh automatically according to lambda')
...
# Casting and defaut values 
T            = float(args.tf) if args.tf is not None                   else 5.0
time_steps   = int(args.n) if args.n          is not None              else 4
kmax         = int(args.kmax) if args.kmax is not None                 else 500
res_dir      = str(args.o) if args.o is not None                       else './results/default/'
rho          = float(args.rho) if args.rho is not None                 else 1.0
maxiter      = int(args.maxiter) if args.maxiter is not None           else 10
odesolver    = str(args.odesolver) if args.odesolver is not None       else 'euler'
epstol       = float(args.epstol) if args.epstol is not None           else 1e-2 # unused

# adapt settings
adapt_max_layers = int(args.maxlayers) if args.maxlayers is not None   else 32
adapt_add_layers = int(args.addlayers) if args.addlayers is not None   else 10

# strategy
adapt_auto   = int(args.adaptauto) if args.adaptauto is not None       else 0   # 0/1 : disable/enable
adapt_each   = int(args.adapteach) if args.adapteach is not None       else 0   # adapt each # steps
adapt_dyadic = int(args.adaptdyadic) if args.adaptdyadic is not None   else 0   # adapt each # steps
adapt_rho    = int(args.adaptrho) if args.adaptrho is not None         else 0   # adapt each # steps

# for 2D oscillator and neural net
eta          = float(args.eta) if args.eta is not None                 else 0.0
abn_mul      = 1.0  # abnormal multiplier

# for neural net only
d            = int(args.d) if args.d is not None                       else 2
K            = int(args.K) if args.K        is not None                else 50
no_avg_iters = int(args.navg) if args.navg is not None                 else 1

prb          = str(args.problem) if args.problem is not None           else 'sinus'
...
# Init and train with EMSA
settings = {
  'adapt_each'           : adapt_each,        # adaptive mode : adapt arbitraly each #
  'adapt_auto'           : adapt_auto,        #                 automatic adaptativity based on slope of lambda
  'adapt_max_nodes'      : adapt_max_layers,               #                 maximum number of layers
  'adapt_add_layers'     : adapt_add_layers,               #                 number of layer to add @ each refinement
...
if comm.rank == 0:
  ...
  print('{:50} {:.2E}'.format('epstol',              epstol))
  print('--------------------------------------------------------------')
